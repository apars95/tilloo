...
class Job extends EventEmitter {
    ...
    async start() {
        ...
        const scriptPath = self.command[0];
        const args = self.command.slice(1);
        let output;

        if (args.length > 0) {
            const argumentList = args.map((arg) => `--${arg}`).join(' ');
            output = `Script used: ${scriptPath} ${argumentList}`;
        }
        else {
            output = `Script used: ${scriptPath}`;
        }

        if (output && output.length > 0) {
            iostatus.sendLogOutput(self.runId, output);
        }
